...
LitigationBIDirectors.getCompanyDetailsByKid = async (req, res) => {
  try {
    ...
    if (litigationBiId) {
      let foundLitigationDetails = await KarzaLitigationBiDetails.findById(litigationBiId);
    ...
    let companyDetails = await KarzaLitigationBiDirectors.findOne(query);
    ...
    await saveLog(req.body.jwtInfo.jwtId, "Litigation BI Directors", responseTime, response.data.statusCode === 200 ? "success" : "failed", req.body);
