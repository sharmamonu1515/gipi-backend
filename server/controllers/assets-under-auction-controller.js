...
AssetsUnderAuction.search = async (req, res) => {
  try {
    ...
    const response = await axios.post(`${await Karza.getAPIBaseURL()}/v3/bank-auction`, requestBody, {
      headers: {
    ...
    await saveLog(req.body.jwtInfo.jwtId, "Assets Under Auction", responseTime, response.data.statusCode === 101 ? "success" : "failed", requestBody);
