...
BIFR.search = async (req, res) => {
  try {
    ...
    await saveLog(req.body.jwtInfo.jwtId, "BIFR", responseTime, response.data.statusCode === 101 ? "success" : "failed", requestBody);
...
BIFR.getList = async (req, res) => {
  try {
    ...
    const query = search
      ? {
          $or: [{ entityId: { $regex: search, $options: "i" } }, { "data.entityName": { $regex: search, $options: "i" } }, { "data.caseNo": { $regex: search, $options: "i" } }],
        }
      : {};
