...
BankDefaulters.search = async (req, res) => {
  try {
    ...
    const response = await axios.post(`${await Karza.getAPIBaseURL()}/v3/bank-defaulter`, requestBody, {
      headers: {
    ...
    await saveLog(req.body.jwtInfo.jwtId, "Bank Defaulters", responseTime, response.data.statusCode === 101 ? "success" : "failed", requestBody);
