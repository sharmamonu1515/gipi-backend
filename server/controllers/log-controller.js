...
const KarzaLog = require("../models/log");
...
Log.getList = async (req, res) => {
  try {
    ...
    const query = search
      ? {
          $or: [
            { userId: mongoose.Types.ObjectId.isValid(search) ? new mongoose.Types.ObjectId(search) : undefined },
            { apiType: { $regex: search, $options: "i" } },
          ],
        }
      : {};
