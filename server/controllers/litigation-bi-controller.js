...
LitigationBI.getAndSaveAuthToken = async () => {
  try {
    ...
    if (!foundLitigationDetails) {
      ...
      let updatedAuthData = await KarzaLitigationBiAuth.findOneAndUpdate({ username: loginDetails.username }, { token: authDetails.data.data, setCookie: authSetCookie, isExpired: false }, { new: true });
  ...
  try {
    ...
    axios
      ...
      .then(async (response) => {
        ...
        await saveLog(req.user.id, "Litigation BI", responseTime, response.data.statusCode === 200 ? "success" : "failed", req.body);
...
LitigationBI.directorSearch = async (req, res) => {
  try {
    ...
    foundLitigationDetails = foundLitigationDetails.toObject();
    ...
    let foundDirectorDetails = await KarzaLitigationBiDirectorDetails.findOne({
    ...
    axios
      ...
      .then(async (response) => {
        ...
        await saveLog(req.user.id, "Litigation BI Director Details", responseTime, response.data.statusCode === 200 ? "success" : "failed", req.body);
...
LitigationBI.getList = async (req, res) => {
  try {
    ...
    const litigationBIs = await KarzaLitigationBiDetails.find(query, projection)
      .populate({ path: "userId", select: "local.userName" })
      .skip((page - 1) * limit)
      .limit(limit)
      .sort({ [sort]: sortOrder });
...
LitigationBI.exportExcel = async (req, res) => {
  try {
    ...
    const { entityType = "company", id, fileType = "lite", filters = {} } = req.body;
    ...
    if (isEntity) {
      ...
      litigationDetails = litigationDetails.toObject();
    } else {
      // check director
      ...
      litigationDirectorDetails = litigationDirectorDetails.toObject();
    ...
    const storageDir = path.join(__dirname, `../../client/public/uploads/excel_files/${id}`);
    ...
    let fileName = `litigation_${id}_${fileType}`;

    if (filters) {
      const filterArr = [];
      if (filters.caseStatus) {
        hasFilters = true;
        filterArr.push(filters.caseStatus.replace(" ", "-"));
        additionalBodyParams.caseStatus = filters.caseStatus;
      }

      if (filters.caseTypeCategory) {
        hasFilters = true;
        filterArr.push(filters.caseTypeCategory.replace(" ", "-"));
        additionalBodyParams.caseTypeCategory = filters.caseTypeCategory;
      }

      if (filters.severity && filters.severity.length) {
        hasFilters = true;
        filterArr.push(filters.severity.join("-"));
        additionalBodyParams.severity = filters.severity;
      }

      if (filters.relevance) {
        hasFilters = true;
        filterArr.push(filters.relevance.replace(" ", "-"));
        additionalBodyParams.severity = [filters.relevance];
      }

      if (filters.upcomingHearing) {
        hasFilters = true;
        filterArr.push(filters.upcomingHearing.replace(" ", "-"));
        additionalBodyParams.upcomingPeriod = filters.upcomingHearing;
      }

      fileName = fileName + "_" + filterArr.join("_");
    }
    ...
    if (fs.existsSync(filePath)) {
      return res.send({
        ...
        downloadLink: `${process.env.LITIGATION_FILE_PATH}/${id}/${fileName}`,
      });
    }
    ...
    const reqData = {
      ...
      searchType: isEntity ? "ENTITY" : "INDIVIDUAL",
      pageSize: LitigationBI.PAGE_SIZE,
      pageNo: 1,
      ui_response: true,
      version: 3,
    ...
    // **Fetch Auth Token**
    console.log(`Fetching File from API.`, reqData);
    const litigationAuthDetails = await LitigationBI.getAndSaveAuthToken();

    let config = {
      method: "post",
      maxBodyLength: Infinity,
      url: hasFilters ? await Karza.getExcelFiltersURL() : await Karza.getExcelURL(),
      headers: await Karza.getHeaders(litigationAuthDetails.setCookie),
      data: JSON.stringify(reqData),
    };
    ...
    writer.on("finish", async () => {
      ...
      const downloadLink = `${process.env.LITIGATION_FILE_PATH}/${id}/${fileName}`;
      if (isEntity) {
        await KarzaLitigationBiDetails.findByIdAndUpdate(litigationDetails._id, {
          $addToSet: { downloadLink },
        });
      } else {
        await KarzaLitigationBiDirectorDetails.findByIdAndUpdate(litigationDirectorDetails._id, {
          $addToSet: { downloadLink },
        });
      }

      const responseTime = Date.now() - startTime;
      await saveLog(req.user.id, `Litigation BI ${isEntity ? "" : "Director "}Excel (${fileType})`, responseTime, "success", reqData);
