...
const countryInfo = require('../controllers/countries-controller');
let countryDetails = {
    shortName: 'IN'
};

const companyInSchema = new mongoose.Schema({
    cin: String,
    country: String,
    name: {
        type: String,
...
companyInSchema.index({ name: 1 });

companyInSchema.statics.addInCompany = async function addInCompany(data) {
    try {

        countryDetails = await countryInfo.getCountriesInfo(data.country);
        ...
        let Company = mongoose.model(`${countryDetails.shortName.toLowerCase()}_companies`, companyInSchema);
...
companyInSchema.statics.returnCountrySchema = async function returnCountrySchema(data) {
    return mongoose.model(`${data.toLowerCase()}_companies`, companyInSchema);
}

module.exports = mongoose.model(`${countryDetails.shortName.toLowerCase()}_companies`, companyInSchema);
// module.exports.schema = companyInSchema;